...
export function TabletTable({ data, onEdit, onRefresh }: { data: any[], onEdit: (item: any) => void, onRefresh: () => void }) {
  ...
  const handleBatchDelete = async () => {
    try {
      await Promise.all(Array.from(selectedIds).map(id => fetch('/api/tablets/' + id, { method: 'DELETE' })))
      ...
      <div className="overflow-auto max-h-[calc(100vh-16rem)] relative">
        ...
        <TableHeader className="sticky top-0 bg-white/95 backdrop-blur z-10 shadow-sm">
          <TableRow>
            ...
            <SortableTableHead label="Form" sortKey="form_path" currentSort={sortConfig} onRequestSort={requestSort} currentFilter={columnFilters['form_path']} onFilterChange={setColumnFilter} data={data} />
  ...
  <TableCell className="sticky right-0 bg-white/90 backdrop-blur shadow-[-4px_0_12px_rgba(0,0,0,0.02)] z-10">
                <div className="flex gap-1 justify-center">
                  <AssetTransferActions item={item} tableName="tablets" onSuccess={onRefresh} />
          ...
          {data.length === 0 && (
            <TableRow><TableCell colSpan={20} className="p-0"><EmptyState /></TableCell></TableRow>
          )}
        </TableBody>
      </Table>
      </div>
