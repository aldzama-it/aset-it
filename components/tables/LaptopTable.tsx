...
export function LaptopTable({ data, onEdit, onRefresh }: { data: any[], onEdit: (item: any) => void, onRefresh: () => void }) {
  ...
  const handleBatchDelete = async () => {
    try {
      await Promise.all(Array.from(selectedIds).map(id => fetch('/api/laptops/' + id, { method: 'DELETE' })))
...
Tim IT`)
  ...
  <TableCell>
          ...
          {data.length === 0 && (
            <TableRow><TableCell colSpan={22} className="p-0"><EmptyState /></TableCell></TableRow>
          )}
        </TableBody>
      </Table>
      </div>
