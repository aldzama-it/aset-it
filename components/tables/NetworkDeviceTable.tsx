...
export function NetworkDeviceTable({ data, onEdit, onRefresh }: { data: any[], onEdit: (item: any) => void, onRefresh: () => void }) {
  ...
  const handleBatchDelete = async () => {
    try {
      await Promise.all(Array.from(selectedIds).map(id => fetch('/api/network/' + id, { method: 'DELETE' })))
  ...
  <TableCell>
                <div className="flex gap-1 justify-center">
                  <AssetTransferActions item={item} tableName="networkDevices" onSuccess={onRefresh} />
          ...
          {data.length === 0 && (
            <TableRow><TableCell colSpan={9} className="p-0"><EmptyState /></TableCell></TableRow>
          )}
        </TableBody>
      </Table>
      </div>
