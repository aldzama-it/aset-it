...
export function DashcamTable({ data, onEdit, onRefresh }: { data: any[], onEdit: (item: any) => void, onRefresh: () => void }) {
  ...
  const handleBatchDelete = async () => {
    try {
      await Promise.all(Array.from(selectedIds).map(id => fetch('/api/dashcams/' + id, { method: 'DELETE' })))
  ...
  <TableCell>
                <div className="flex gap-1 justify-center">
                  <AssetTransferActions item={item} tableName="dashcams" onSuccess={onRefresh} />
