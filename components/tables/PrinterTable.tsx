...
export function PrinterTable({ data, onEdit, onRefresh }: { data: any[], onEdit: (item: any) => void, onRefresh: () => void }) {
  ...
  const handleBatchDelete = async () => {
    try {
      await Promise.all(Array.from(selectedIds).map(id => fetch('/api/printers/' + id, { method: 'DELETE' })))
  ...
  const viewFields: ViewField[] = [
    ...
    { label: 'Tipe Tinta', key: 'ink_type' },
    { label: 'Koneksi', key: 'connection' },
      ...
      <div className="overflow-auto max-h-[calc(100vh-16rem)] relative">
        ...
        <TableHeader className="sticky top-0 bg-white/95 backdrop-blur z-10 shadow-sm">
          <TableRow>
            ...
            <SortableTableHead label="Tipe Tinta" sortKey="ink_type" currentSort={sortConfig} onRequestSort={requestSort} currentFilter={columnFilters['ink_type']} onFilterChange={setColumnFilter} data={data} />
            <SortableTableHead label="Koneksi" sortKey="connection" currentSort={sortConfig} onRequestSort={requestSort} currentFilter={columnFilters['connection']} onFilterChange={setColumnFilter} data={data} />
        ...
        <TableBody>
          {paginatedData.map((item: any) => (
            <React.Fragment key={item.id}>
              ...
              <TableCell className="whitespace-nowrap">{item.ink_type || "-"}</TableCell>
              <TableCell className="whitespace-nowrap">{item.connection || "-"}</TableCell>
