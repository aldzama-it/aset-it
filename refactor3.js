...
const files = fs.readdirSync(tablesDir).filter(f => f.endsWith('Table.tsx') && !f.includes('DigitalAsset') && f !== 'CameraTable.tsx')
...
files.forEach(file => {
  ...
  // Find the action buttons block specifically using [^\<]* to ensure we only get that block
  const actionMatch = content.match(/(<TableCell[^>]*>\s*<div className="flex gap-1 justify-center">[\s\S]*?<\/TableCell>)/)
