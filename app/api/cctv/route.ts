...
export async function GET(req: Request) {
  ...
  try {
    const allData = await prisma.cctv.findMany({
...
export async function POST(req: Request) {
  try {
    ...
    if (!asset_code) {
      const last = await prisma.cctv.findFirst({ orderBy: { id: 'desc' } })
      asset_code = generateAssetCode('CTV', last?.asset_code)
    }
    const data = await prisma.cctv.create({
      data: { ...body, asset_code , updated_at: new Date() }
    })
    await recordHistory({
      table_name: 'cctvs', asset_id: data.id, asset_code,
