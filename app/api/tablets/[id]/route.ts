...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const before = await prisma.tablet.findUnique({ where: { id: +resolvedParams.id } })
    const data = await prisma.tablet.update({
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const before = await prisma.tablet.findUnique({ where: { id: +resolvedParams.id } })
    await recordHistory({
      table_name: 'tablets', asset_id: +resolvedParams.id, asset_code: before?.asset_code,
      action: 'Dihapus', old_condition: before?.condition
    })
    await prisma.tablet.delete({ where: { id: +resolvedParams.id } })
