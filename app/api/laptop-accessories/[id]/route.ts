...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const before = await prisma.laptopAccessory.findUnique({ where: { id: +resolvedParams.id } })
    const data = await prisma.laptopAccessory.update({
      where: { id: +resolvedParams.id }, data: { ...body, updated_at: new Date() }
    })
    await recordHistory({
      table_name: 'laptopAccessorys', asset_id: data.id, asset_code: data.asset_code,
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const before = await prisma.laptopAccessory.findUnique({ where: { id: +resolvedParams.id } })
    await recordHistory({
      table_name: 'laptopAccessorys', asset_id: +resolvedParams.id, asset_code: before?.asset_code,
      action: 'Dihapus', old_condition: before?.condition
    })
    await prisma.laptopAccessory.delete({ where: { id: +resolvedParams.id } })
