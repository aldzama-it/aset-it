...
import { Plus, Download, Laptop, Laptop2, MonitorX, AlertTriangle } from 'lucide-react'
...
export default function LaptopPage() {
  ...
  const [summary, setSummary] = useState({ total: 0, dipakai: 0, tersedia: 0, rusak: 0 })
  const [search, setSearch] = useState('')
  const [statusFilter, setStatusFilter] = useState('all') // 'all', 'Aktif', 'Rusak', 'Tersedia'
  ...
  const filteredData = data.filter(item => {
    ...
    if (statusFilter === 'Aktif' || statusFilter === 'Dipakai') return item.status_virtual === 'Aktif' || item.status_virtual === 'Dipakai'
    return true
  })
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
        ...
        <Card
          className={`py-3 px-1 bg-card/80 backdrop-blur-sm border-border/60 cursor-pointer transition-all hover:border-red-500/50 ${statusFilter === 'Rusak' ? 'ring-2 ring-red-500 border-transparent' : ''}`}
