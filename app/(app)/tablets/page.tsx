...
import { Plus, Download, Tablet, AlertTriangle, CheckCircle2 } from 'lucide-react'
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
        <Card className={`py-3 px-1 bg-card/80 backdrop-blur-sm border-border/60 cursor-pointer transition-all hover:border-primary/50 ${statusFilter === 'all' ? 'ring-2 ring-primary border-transparent' : ''}`} onClick={() => setStatusFilter('all')}>
          <CardContent className="p-0 flex items-center gap-3 px-4">
            <div className="p-2 bg-primary/10 rounded-lg"><Tablet className="w-5 h-5 text-primary" /></div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0">
          <Button variant="outline" onClick={() => exportToExcel(data, 'Data_Tablet')} className="shrink-0">
      ...
      <TabletTable data={filteredData} onEdit={(item) => { setEditItem(item); setFormOpen(true) }} onRefresh={fetchData} />
