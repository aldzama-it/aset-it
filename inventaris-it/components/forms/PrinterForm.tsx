...
export function PrinterForm({ open, onOpenChange, item, onSuccess }: { open: boolean, onOpenChange: (o: boolean) => void, item: any, onSuccess: () => void }) {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const url = item ? '/api/printers/' + item.id : '/api/printers'
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{item ? 'Edit' : 'Tambah'} Printer</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label className="capitalize">ink type</Label>
              <Input type="text" {...register('ink_type')} />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <Label className="capitalize">connection</Label>
